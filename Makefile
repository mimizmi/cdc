.PHONY: build up down test test-unit test-integration test-fault lint \
...
up:
	...
	@echo "等待服务就绪 (约 30s)..."
	@sleep 30
	@docker compose exec mysql mysql -uroot -proot hospital_his -e "SHOW TABLES;" 2>/dev/null && echo "MySQL ready" || echo "MySQL not ready yet"
...
restart:
	docker compose restart cdc-platform
